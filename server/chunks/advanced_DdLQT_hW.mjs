...
const $$Advanced = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const entitlement = await (await import("./client_BQ4VG-sy.mjs")).cachedEntitlement(env);
  ...
  const { getAutonomyConfig, ghDeviceAvailable } = await import("./autonomy_BmbXwvAd.mjs");
  ...
  const { storageMode, maxUploadMb } = await import("./storage_DPfGoCa3.mjs");
