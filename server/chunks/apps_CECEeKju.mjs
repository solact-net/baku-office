...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { atLeast, planLabel } = await import("./ctx_D8V0BLlh.mjs").then((n) => n.J);
  const entitlement = await (await import("./client_BQ4VG-sy.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_DMjXyAVt.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_W2mhsSSD.mjs");
  const { registeredParts } = await import("./parts_D92YfUNk.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers, isRegisteredPart, reconcileInstalledParts } = await import("./external-apps_BhUnfGrW.mjs").then((n) => n.B);
  const { storeCatalog, myApps } = await import("./store_CU2q746t.mjs");
