...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec, hasApiKey } = await import("./client_BQ4VG-sy.mjs");
  const AI_PROVIDERS = [
  ...
  await import("./index_DMjXyAVt.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_W2mhsSSD.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_hzjOCpN9.mjs");
  ...
  const { reviewQueue } = await import("./users_-D4262Ri.mjs");
  const { backupAlert } = await import("./backup_BCoVnJg1.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_BhUnfGrW.mjs").then((n) => n.B);
  ...
  const { storageMode } = await import("./storage_DPfGoCa3.mjs");
